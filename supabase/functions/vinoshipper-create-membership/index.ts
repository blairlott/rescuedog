...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 3. Save IDs back to our row
    await supabase
      .from("wine_club_memberships")
      .update({
        // These columns are added in the upcoming migration:
        // vinoshipper_customer_id, vinoshipper_membership_id
        // (Cast to any until types regenerate)
      } as Record<string, unknown>)
      .eq("id", membership.id);

    return json({
      ok: true,
      vinoshipperCustomerId: customer.id,
