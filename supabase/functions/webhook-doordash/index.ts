// DoorDash purchase webhook.
// Header: X-DoorDash-Signature (sha256 hex). Secret: DOORDASH_WEBHOOK_SECRET.
...
Deno.serve(async (req) => {
  ...
  const ok = await verifyHmac(
    raw, sig, Deno.env.get("DOORDASH_WEBHOOK_SECRET"),
  ...
  try {
    ...
    console.error("[webhook-doordash]", e);
