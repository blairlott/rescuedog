DROP FUNCTION IF EXISTS public.finance_vs_summary(date, date);

CREATE FUNCTION public.finance_vs_summary(_start date, _end date)
 RETURNS TABLE(
   order_count bigint,
   ...
   aov_cents bigint,
   wine_club_cents bigint,
   ala_carte_cents bigint,
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(SUM(ROUND((COALESCE(t.gross_value,0) - COALESCE(t.discount,0)) * 100)),0)::bigint,
